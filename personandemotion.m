...
% PERSONANDEMOTION MATLAB code for personandemotion.fig
%      PERSONANDEMOTION, by itself, creates a new PERSONANDEMOTION or raises the existing
%      singleton*.
%
%      H = PERSONANDEMOTION returns the handle to a new PERSONANDEMOTION or the handle to
%      the existing singleton*.
%
%      PERSONANDEMOTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PERSONANDEMOTION.M with the given input arguments.
%
%      PERSONANDEMOTION('Property','Value',...) creates a new PERSONANDEMOTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before personandemotion_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to personandemotion_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help personandemotion

% Last Modified by GUIDE v2.5 13-Apr-2013 00:36:01
...
if(score<=0.0009)
    set(handles.text1,'String', 'No Match Found');
else
...
[emotion_score,emotion] = max(scores1+scores2); %Take the sum of the two scores to calculate emotion
